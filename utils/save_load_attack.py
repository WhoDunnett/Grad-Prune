...
def save_attack_result(
    ...
    bd_train_all: Optional[prepro_cls_DatasetBD_v2] = None, # MUST be dataset without transform
):
    '''
    ...
    save_dict = {
            ...
            'bd_train_all': bd_train_all.retrieve_state() if bd_train_all is not None else None,
...
def load_attack_result(
    ...
    if all(key in load_file for key in ['model_name',
        ...
        print(f"clean_setting.dataset_path: {clean_setting.dataset_path}")
        ...
        if load_file['bd_train_all'] is not None:
            bd_train_all_dataset = prepro_cls_DatasetBD_v2(train_dataset_without_transform)
            bd_train_all_dataset.set_state(
                load_file['bd_train_all']
            )
            bd_train_all_dataset_with_transform = dataset_wrapper_with_transform(
                bd_train_all_dataset,
                train_img_transform,
                train_label_transform,
            )
        else:
            logging.info("No bd_train_all info found.")
            bd_train_all_dataset_with_transform = None
        ...
        load_dict = {
                ...
                'bd_train_all': bd_train_all_dataset_with_transform,
